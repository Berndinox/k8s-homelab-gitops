...
| Aspect | Ubuntu (Old) | Fedora CoreOS (New) |
|--------|--------------|---------------------|
| OS Installation | Manual Ubuntu install → Shell scripts | Automated via Ignition config |
| Configuration | Post-install bash scripts | Declarative Butane/Ignition |
| Package Management | apt install packages | Immutable OS, containers only |
| Network Config | Netplan YAML | NetworkManager connections |
| Updates | apt upgrade | rpm-ostree (atomic updates) |
| Kernel Modules | systemd-modules-load.service | Same (Ignition creates config) |
| RKE2 Install | curl script in bash | curl script in systemd service |
| Persistence | Everything mutable | Only /etc and /var are writable |
...
1. **Preparation** (10 minutes)
   - Download Fedora CoreOS ISO
   - Install Butane & coreos-installer
   - Customize SSH keys and passwords

2. **host03 Bootstrap** (30 minutes)
   - Convert Butane → Ignition
   - Create USB with embedded config
   - Boot and install
   - Wait for RKE2 to initialize
   - Save join token

3. **host01 & host02 Join** (20 minutes each)
   - Update Butane configs with join token
   - Create USBs for both nodes
   - Boot and install both nodes
   - Wait for cluster join

4. **ArgoCD & GitOps** (10 minutes)
   - Deploy ArgoCD
   - Bootstrap GitOps applications

**Total time: ~1.5 hours** (mostly waiting for automated processes)

---

## Next Steps

- [ ] Deploy Longhorn storage (via ArgoCD)
- [ ] Configure Multus CNI
- [ ] Deploy KubeVirt
- [ ] Configure BGP peering with OPNsense
- [ ] Deploy applications
